...
export class MessageModel {
    ...
    outputData = () => {
        ...
        this.json.mails.forEach((pars) => {
            ...
            this.messages.push({
                ...
                avatar: `https://${window.location.hostname}${pars.avatar_url}`,
                read: pars.mail.read,
    ...
    getOutComeMessage = async () => {
        try {
            const res = await fetch(`https://${window.location.hostname}/api/v1/mail/outcome`, {
                mode: 'cors',
                headers: {
    ...
    rmMessageInFolder = async (folder_name: string, mail_id: number) => {
        const header = await getCSRFToken(`https://${window.location.hostname}/api/v1/folder/mail/delete`);

        await fetch(`https://${window.location.hostname}/api/v1/folder/mail/delete`, {
            mode: 'cors',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-token': header,
    ...
    selectFolderMessage = async (folder_name: string) => {
        try {
            const res = await fetch(`https://${window.location.hostname}/api/v1/folder/list?folder_name=${folder_name}`, {
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    moveInFolderMessage = async (folder_name_dest: string, folder_name_src: string, mail_id: number) => {
        try {
            const header = await getCSRFToken(`https://${window.location.hostname}/api/v1/folder/mail/move`);
            await fetch(`https://${window.location.hostname}/api/v1/folder/mail/move`, {
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-token': header,
    ...
    addInFolderMessage = async (folder_name: string, mail_id: number) => {
        try {
            const header = await getCSRFToken(`https://${window.location.hostname}/api/v1/folder/mail/add`);
            const res = await fetch(`https://${window.location.hostname}/api/v1/folder/mail/add`, {
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-token': header,
    ...
    getFolders = async () => {
        try {
            const res = await fetch(`https://${window.location.hostname}/api/v1/folder/list`, {
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    getMessage = async (name: string) => {
        if (name === 'income' || name === 'outcome') {
            try {
                const res = await fetch(`https://${window.location.hostname}/api/v1/mail/${name}`, {
                    mode: 'cors',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            try {
                const res = await fetch(`https://${window.location.hostname}/api/v1/folder/list?folder_name=${name}`, {
                    mode: 'cors',
                    headers: {
                        'Content-Type': 'application/json',
                    },
