    ...
    fetchSetAvatar = async (data: {first_name: string, last_name: string, avatar: string}) => {
        try {
            if (data.avatar !== undefined) {
                ...
                const header = await getCSRFToken(`http://${window.location.hostname}:80/api/v1/profile/avatar/set`);
                const postAvatarSet = await fetch(`http://${window.location.hostname}:80/api/v1/profile/avatar/set`, {
                    mode: 'cors',
                    headers: {
                        'X-CSRF-token': header,
                    },
            ...
            const header = await getCSRFToken(`http://${window.location.hostname}:80/api/v1/profile/set`);
            const postSetProfile = await fetch(`http://${window.location.hostname}:80/api/v1/profile/set`, {
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-token': header,
